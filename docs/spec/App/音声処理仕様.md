...
| SoundIsolation | `VoiceIsolationRuntimeSnapshot` | 送信、RX master、peer bus の support / active / bypassed 表示 |
| DynamicsProcessor | `DynamicsProcessorRuntimeSnapshot` | TX 音量整形 stage の support / active 表示 |
...
| UI | Diagnostics は `AudioMixerSnapshot` 由来の `transmitEffectChainSnapshot.stages` を描画する。固定の `SoundIsolation -> VADGate -> DynamicsProcessor -> PeakLimiter` 文字列に依存しない |
...
受信側は RTC / Codec で PCM に戻した peer ごとの音声を AudioMixer の peer bus に入れる。相手は 0...n 人で、peer ごとに RTC 接続状態、認証状態、decode codec、AudioMixer peer bus を別々の診断層として持つ。各 peer bus は個別音量と ordered effect chain を持ち、その後に全 peer bus を receive mix down で 1 本の receive master bus へまとめる。receive master bus は全体音量と ordered effect chain を持ち、最後に SessionManager の output renderer へ渡す。peer bus、route、master bus、output bus は `AudioMixerSnapshot` から確認する。

| 段階 | 処理 |
|---|---|
| RTC receive | RTC から peer ごとの `ReceivedAudioPacket` を受け取る |
| Codec decode | `ReceivedAudioPacket.packet` を Codec で `AudioCore.PCMFrame` へ戻し、decode 失敗は drop として扱う |
...
| master effect chain | receive master bus の ordered effect chain を先頭から順に適用する。最後は PeakLimiter で過大ピークを抑える |
| SessionManager output | 最終 output stream へ渡し、schedule report と level を Diagnostics に反映する |

Diagnostics は受信側を単一 peer の直列処理として見せない。`RTC RX` は誰が接続・認証されているか、`Decode` は peer ごとの `activeCodec`、`Peer Buses` は RX / JIT / PLAY / level / volume / `receivePeerEffectChainSnapshot(peerID:)` を一覧し、`Mix Down` で `AudioMixerSnapshot.routes` 上の peer bus が receive master bus へ集約されることを見せる。decode codec は peer の keepalive metadata で受け取った `activeCodec` を使い、metadata 未到達時は `Unknown` として表示する。

### 受信用 effect chain

受信用 effect chain は、peer bus と receive master bus の 2 箇所にそれぞれ順序付き stage 配列として持つ。Diagnostics は `AudioMixerSnapshot` 由来の `receivePeerEffectChainSnapshot(peerID:)` と `receiveMasterEffectChainSnapshot` を描画するだけで、effect の既定値や stage 配列を持たない。

| chain | stage | package | 役割 | Diagnostics 表示 |
|---|---|---|---|---|
| peer effect chain | `soundIsolation` | SoundIsolation | peer 単位で受信音声の voice isolation を必要に応じて行う | `receive-peer-{index}-effect-stage-sound-isolation` |
...
| master effect chain | `peakLimiter` | PeakLimiter | RX master の最終段で過大ピークを抑える。常に最後へ挿入する | `receive-master-effect-stage-peak-limiter` |

| 追加時のルール | 仕様 |
|---|---|
| 追加位置 | peer 単位で効かせたい effect は peer effect chain、受信全体へ効かせたい effect は master effect chain へ追加する。master effect chain の最後は PeakLimiter 固定 |
| UI | Diagnostics は `AudioMixerSnapshot` 由来の chain snapshot を描画する。固定の `SoundIsolation` 文字列や default 値に依存しない |
| 設定 | ユーザーに出す設定は App 設定値一覧に追加する。Audio Unit parameter や effect index は通常 UI に出さない |
