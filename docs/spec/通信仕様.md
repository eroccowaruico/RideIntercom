...
| `startLocalStandby()` | 音声未開始のまま `callSession.startStandby(group:)` を呼び、近距離ピア待受だけを開始 | すぐ話し始めなくても Local 発見は先行させる |
...
```mermaid
flowchart TD
    A[OutboundAudioPacket] --> B{connectionState}
    B -- localConnecting/localConnected --> C[CallSession]
    C --> D[RouteManager]
    D --> E[MultipeerLocalRoute]
    E --> F[Multipeer unreliable packet audio]
    B -- idle/reconnectingOffline --> G[送信しない]
```
