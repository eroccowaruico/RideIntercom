...
| インターネット通信 | WebSocket adapter 経由の通信 |
...
| Internet Transport層 | InternetTransport + InternetTransportAdapter | WebSocket送受信 |
...
| transmitMetadata | requestedCodec + encodedCodec + fallbackReason（voice/keepalive共通で付与） |

### transmitMetadata 仕様

| フィールド | 内容 |
|---|---|
| requestedCodec | 送信側で要求したcodec（Settingsで選択した値、またはその時点の送信要求値） |
| encodedCodec | 実際にエンコードされたcodec |
| fallbackReason | null / codecUnavailable / encoderReturnedEmptyPayload / encodingFailed |

| 送信タイミング | 仕様 |
|---|---|
| 音声/keepalive送信時 | 各AudioPacketEnvelopeに transmitMetadata を同梱 |
| 接続イベント（authenticated） | 状態同期用 keepalive を送信し、最新 transmitMetadata を通知 |
| ローカル操作イベント | ミュート切替・送信codec切替・HE-AAC品質変更時に状態同期用 keepalive を送信 |

| 判定 | 仕様 |
|---|---|
| 正常一致 | requestedCodec == encodedCodec かつ fallbackReason == null |
| フォールバック | requestedCodec != encodedCodec または fallbackReason != null |
| メタデータ不整合 | 受信側で encodedVoice.codec と transmitMetadata.encodedCodec が不一致のとき診断イベントとして記録 |

補足: メタデータ不整合やフォールバックが発生しても通信は継続する。受信破棄やアプリ終了は行わず、Diagnosticsと運用ログで可視化する。
