...
RideIntercom全体に対して、方針逸脱を「発見して直す」運用から「逸脱が入らない」構造へ移行する。中核は、機能契約の固定、共通経路の単一化、Adapter境界の明文化、テスト先行ゲート、設計責務の再配置の5本柱とする。既存の抽象化基盤（Transport/AudioEncoding/RuntimeFactory）を再利用しつつ、未完了境界（Opus/Internet handover/macOS実経路）と不十分な抽象境界を設計段階で閉じる。
...
- 完了済み（Step 13/26 の一部）: `RouteCoordinator`/`RoutePolicy` 導入、InternetTransport を共通 packet contract 化、InternetTransportAdapter 境界を追加
- 完了済み（Step 25/28 の一部）: stateful codec session 境界導入、credential/handshake/packet crypto 責務分離、invite 生成/受理の UseCase 分離、Opus backend 差し替え境界の実装、Application/UI 層の OS 分岐逆流防止テスト追加、System Opus backend 導入トグル追加
- 完了済み（Step 25/26/28 の追加）: Internet adapter 選択条件を ws/wss + host に固定しフォールバック規則をテスト化、Opus backend env toggle 契約をテスト化、招待受理時の metadata/secret 永続化責務分離をテストで固定
- 進捗コミット: `98e3f23` `c32ac93` `4bcdb7d` `58edd0f` `70f76d1` `036eafb` `ddf6093` `38cb5d5` `209a7f6` `252d8eb` `d06422a` `e35fa0b` `f82f1cc` `f40f21e` `b97a53d` `569faaa` `265c00f`
- 次アクション: Step 25-28 の未達（System Opus backend の実環境接続検証、Internet relay 実サーバ接続検証、Platform 層差分契約テスト拡充）を順次実装
...
7. 音声送受信と状態更新の単一経路を明文化し、入口を [RideIntercom/IntercomCore.swift](../../RideIntercom/IntercomCore.swift) 周辺のイベント処理契約へ集約する。入力は「MicrophoneFrame」「TransportEvent」「UserIntent」、出力は「RenderedAudio」「UIState」「DiagnosticsSnapshot」に限定し、契約外更新を禁止する。*depends on 6*
...
11. コーデック境界を stateful へ拡張する設計を確定する。対象は [RideIntercom/IntercomCore.swift](../../RideIntercom/IntercomCore.swift) の AudioEncoding、[RideIntercom/IntercomCore.swift](../../RideIntercom/IntercomCore.swift) の AudioEncodingSelector、[RideIntercom/IntercomCore.swift](../../RideIntercom/IntercomCore.swift) の EncodedVoicePacket。設計上は `AudioEncoderSession` と `AudioDecoderSession` を分離し、frame accumulation, flush, reset を契約化する。*depends on 10*
...
15. OS差分はPlatform層に限定し、UIとApplication層から #if を排除する設計へ段階移行する。対象は [RideIntercom/ContentView.swift](../../RideIntercom/ContentView.swift)、[RideIntercom/PlatformAudioInputSupport.swift](../../RideIntercom/PlatformAudioInputSupport.swift)、[RideIntercom/PlatformAudioSessionSupport.swift](../../RideIntercom/PlatformAudioSessionSupport.swift)。各Platform実装は同一Portを満たすAdapterとして定義し、利用側は能力フラグだけを見る。*depends on 6, parallel with 11-14*
...
18. Red-Green-Refactorの順序を必須化し、機能追加ごとに先行テストテンプレートを作成する。優先ケースは Opus fallback、Internet handover、複数peer並行、再認証、ジッタ期限切れ。*depends on 3,6-16*
...
20. 単体テストは ViewModel中心からユースケース・ドメインサービス中心へ移し、ViewModelは状態写像の薄いテストへ縮小する。中心は [RideIntercomTests/RideIntercomTests.swift](../../RideIntercomTests/RideIntercomTests.swift) だが、最終的には責務別ファイルへ分割する。*depends on 19*
21. UIテストを「1回起動で複数ケース」に固定し、接続導線、診断導線、招待導線、削除導線の回帰セットを標準化する。対象は [RideIntercomUITests/RideIntercomUITests.swift](../../RideIntercomUITests/RideIntercomUITests.swift)。*depends on 18, parallel with 19-20*
...
- Platform Adapter層: `AudioInputPort` `AudioOutputPort` `TransportPort` `SecretStorePort` `ClockPort` を実装し、AVFoundation/MC/CoreAudio/Keychain/Internet transport を隔離する。
...
3. Opus/Internet handoverの順序は並行可能だが、品質リスク低減の観点でOpus先行を推奨